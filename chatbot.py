...
def chat():
  while True:
    ...
    if re.compile('|'.join(words1),re.IGNORECASE).search(usermessage):
      print("Is your Storage Heater, Boiler, Insulation or Heat Pump not working properly? You can visit our website https://www.edfenergy.com/for-home/help-support/help-centre to get relevant information on how to fix the issue. If the issue persist you can visit this website https://www.edfenergy.com/heating/boilers/new-boiler.")
      flag=0
      break
    elif re.compile('|'.join(words2),re.IGNORECASE).search(usermessage):
      print("Is your Smart in-home device, in-home display thermostat or HeatSmart/Netamo smart thermostat Boiler not working properly? You can visit our website https://www.edfenergy.com/for-home/help-support/help-centre to get relevant information on how to fix the issue.")
      flag=0
      break
    elif re.compile('|'.join(words3),re.IGNORECASE).search(usermessage):
      print("Do you have an issue with supply disruption or need a new connection? You can visit our website https://www.edfenergy.com/for-home/help-support/help-centre to get relevant information on how to fix the issue.")
      flag=0
      break
    elif re.compile('|'.join(words4),re.IGNORECASE).search(usermessage):
      print("Do you have any issue in MyAccount or need a new connection? You can visit our website https://www.edfenergy.com/for-home/help-support/help-centre to get relevant information on how to fix the issue.")
      flag=0
      break
    elif re.compile('|'.join(words5),re.IGNORECASE).search(usermessage):
      print("Do you have any issue with Payments, Refunds, Energy Bills or want to know about Support Scheme?  You can visit our website https://www.edfenergy.com/for-home/help-support/help-centre to get relevant information on how to fix the issue.")
      flag=0
      break
    elif re.compile('|'.join(words6),re.IGNORECASE).search(usermessage):
      print("Do you have any issue with Paying your bills or want to know to reduce energy costs/Maximize household income ?  You can visit our website https://www.edfenergy.com/for-home/help-support/help-centre to get relevant information on how to fix the issue.")
      flag=0
      break
    elif re.compile('|'.join(words7),re.IGNORECASE).search(usermessage):
      print("We are sorry to hear this. We'd like to offer our support and help. Although I am just a robot, we can assist you by encouraging you to seek professional help. You are eligible for our priority services. Please visit https://www.ofgem.gov.uk/information-consumers/energy-advice-households/getting-extra-help-priority-services-register to register. These are the resources: https://www.nhs.uk/nhs-services/mental-health-services/ . You can get in touch with them ")
      break
